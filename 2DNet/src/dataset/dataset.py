...
class RSNA_Libauc_adapter:
    def __init__(self,
        ...
        self.label = []
        self.length = len(self.name_list) * 4
        self.study_index = np.zeros(self.length, dtype=np.int64)
        for idx in range(self.length):
            ...
            self.study_index[idx] = study_index
...
class RSNA_Dataset_train_by_study_context_Trisampler(data.Dataset):
    def __init__(self,
                 df = None,
                 name_list = None,
                 transform = None
                 ):
        self.adapter = RSNA_Libauc_adapter(df, name_list)
        self.df = df[df['study_instance_uid'].isin(name_list)]
        self.name_list = name_list
        self.transform = transform
        self.targets = self.adapter.label

    def __getitem__(self, idx):
        idx, task_id = idx
        study_name = self.name_list[idx % len(self.name_list)]
        study_train_df = self.df[self.df['study_instance_uid']==study_name]
        study_index = self.adapter.study_index[idx]
        ...
        return image_cat, label, idx, task_id
...
def generate_dataset_loader(df_all, c_train, train_transform, train_batch_size, c_val, val_transform, val_batch_size, workers, use_aucmloss):
    val_dataset = RSNA_Dataset_val_by_study_context(df_all, c_val, val_transform)
    if use_aucmloss:
        train_dataset = RSNA_Dataset_train_by_study_context_Trisampler(df_all, c_train, train_transform)
        sampler = TriSampler(train_dataset, train_batch_size, sampling_rate=0.5)
