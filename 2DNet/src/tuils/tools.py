...
def computeAUROCLibauc(dataGT, dataPRED, classCount):
    ...
    datanpPRED = dataPRED.cpu().numpy()
    for i in range(classCount):
        outAUROC.append(auc_roc_score(datanpGT[:, i], datanpPRED[:, i]))

    return outAUROC

def computeAUPRCLibauc(dataGT, dataPRED, classCount):
    ...
    datanpPRED = dataPRED.cpu().numpy()
    for i in range(classCount):
        outAUPRC.append(auc_prc_score(datanpGT[:, i], datanpPRED[:, i]))
